...
export async function createReauthChallenge(
  adminUserId: string,
  actionType: ReauthActionType,
  targetId?: string,
) {
  return prisma.reauthChallenge.create({
    data: { adminUserId, actionType, targetId: targetId ?? null },
  });
}
...
export async function verifyReauth(
  ...
  await prisma.reauthChallenge.create({
    data: {
      ...
      verifiedAt: new Date(),
      verificationMethod: AUTH_METHOD.PASSWORD_TOTP,
    },
  });
  return { ok: true };
}
