...
beforeEach(async () => {
  ...
  const a = await prisma.adminUser.create({
    data: { email: 'asess@test.local', passwordHash: 'x', name: 'A' },
  });
  adminId = a.id;
});
afterAll(async () => {
...
describe('admin session store', () => {
  it('creates and validates', async () => {
    const { sessionId } = await createAdminSession(adminId, {
      ip: null,
      country: null,
      userAgent: null,
    });
  ...
  it('keeps at most 3 concurrent (evicts oldest)', async () => {
    ...
    for (let i = 0; i < 4; i++)
      ids.push(
        (await createAdminSession(adminId, { ip: null, country: null, userAgent: null })).sessionId,
      );
