...
| C14 | 로케일·표시통화 | 중국어 2종 모두 **번체** 확정 — `zh-TW`(대만·TWD)·`zh-HK`(홍콩·HKD). 간체(`zh-Hans`·본토·CNY) 미지원 → GFW 대응 불필요. 표시통화 ko=KRW/en=USD/ja=JPY/zh-TW=TWD/zh-HK=HKD (v0.2 Stage 3 확정) |
...
├─ display_currency(enum DisplayCurrency: KRW|USD|JPY|TWD|HKD — C17), display_amount, exchange_rate_at_payment
...
1. **PRD v0.2 정합 검증 완료** — 본 문서 (C1–C17 정정 반영 완료. C14는 로케일 정정 — 내용 확인 후 보강 필요).
