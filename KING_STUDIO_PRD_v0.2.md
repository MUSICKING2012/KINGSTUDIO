...
> **이 문서에 대하여**
> 본 문서는 v0.1·v0.2 단계의 다회 논의에서 합의된 모든 결정을 단일 문서로 통합하고, 장 간 교차 참조 충돌 17건(C1–C17)을 정정한 정합 검증본이다. "완결"은 설계 합의가 끝났음을 의미하며, 개발 착수 전 가격 모델링(미해결 이슈 O3) 완료가 선행되어야 한다.
...
| C10 | 체험 인원별 과금 | 인당 +50% 선형: 총액 = 정가 × (1 + 0.5×(인원−1)). 2인 1.5배·3인 2.0배·4인 2.5배·5인 3.0배, 최대 5인. 구 "2인 1인당 반값" 폐기 (v0.2 가격모델 1·2인 검증 + 3~5인 선형 확장) |
...
| C14 | 로케일·표시통화 | 중국어권 2종 — `zh-HK`(번체·홍콩/대만·HKD)·`zh-CN`(간체·중국 본토·CNY). 대만은 별도 로케일을 두지 않고 번체(`zh-HK`)에 통합한다. 간체 로케일은 **지원하되 GFW 대응(ICP 라이선스·본토 CDN·본토 접속 보장)은 비목표** — 타겟은 본토 외부에서 접속 가능한 중국어권 방문객이다(R24). **기본** 표시통화 ko=KRW/en=USD/ja=JPY/zh-HK=HKD/zh-CN=CNY — 사용자 오버라이드 가능(§통화 정책). TWD는 지원 표시통화에서 제외한다. (v0.2 Stage 3 확정 → 2026-07-18 zh-TW 폐지·zh-CN 신설로 개정) |
| C15 | 매직링크 토큰 보안 | raw UUID PK 겸 토큰 → cuid PK + 토큰 SHA-256 해시 저장(원본 미저장). DB 유출 시 활성 링크 비노출. Stage 1 토큰 원칙과 일관 (§5.6) |
| C16 | 곡 카탈로그 | songs title/artist 다국어 → **`song_translations`**(per-locale 행, FTS). MR 라이선스 단일 boolean → **`song_licenses` 종류별**(recording/mr_distribution/lyrics, verified+증빙). R2 "recording OK·MR배포 NG" 표현 가능 (v0.2 Stage 4 확정) |
| C17 | 결제 표시통화 타입 | `display_currency` 자유 String 금지 → **`DisplayCurrency` enum**(KRW/USD/JPY/HKD/CNY, 지원통화만). 환율·영수증 표기 불일치 방지(§4 결제 위험구역). payments·quotes 적용. **2026-07-18 개정**: TWD 제거·CNY 추가 — C14 로케일 개정에 따른 **의도적 정책 변경**이며, 아래 3차 재정정 행에 기록된 04aaded 재퇴행(DisplayCurrency→CNY/EUR 오혼입)과는 무관하다. 되돌리지 말 것. (v0.2 Stage 3 확정 → 2026-07-18 개정) |
...
| — | **인프라 피벗 (2026-07-18)** | 단일 GCP(C13) → **Supabase(DB Postgres·Storage) + Railway(호스팅·트랜스코딩 worker)**. Redis Upstash·Cloudflare Free 유지. 동의 PDF WORM은 비공개 버킷+SHA-256 해시 무결성으로 대체(D1). §5.6·§5.7 갱신 완료, **7장 GCP 세부(§7.3·7.4·7.7·7.13 등)는 구본 — `Infra_Pivot_Decision_v1.md`가 우선, 프로비저닝 시 재작성** |
...
| C20 | blackouts CHECK 제약 + RRULE 형식 가드 (S2.5a) | `blackouts` scope별 필드 유효성 CHECK 추가(DB 최소 가드). ① `slot` — `date_start = date_end` AND `time_start`/`time_end` NOT NULL; ② `full_day` — `date_start <= date_end` AND `time_start`/`time_end` IS NULL; ③ `recurring` — `recurring_rule IS NOT NULL` AND `recurring_rule LIKE 'FREQ=%'` AND `time_start`/`time_end` NOT NULL AND `date_start <= date_end`. **RRULE 가드는 형식 최소 검증** — 빈 문자열·non-RRULE 거부 목적이며 RFC5545 유효성 검증 아님. 미지원 패턴(`FREQ=MONTHLY` 등) 차단은 `isBlackedOut` 파서 `throw` 담당(C20.5). **DB CHECK = 형식 가드 / 파서 = 의미 검증** 역할 분리. `date_start`·`date_end`는 schema NOT NULL이라 CHECK에 IS NOT NULL 가드 불요. (v0.2 확정) |
...
런칭 시점 5개 언어를 모두 지원한다: 한국어, 영어, 일본어, 중국어 번체(홍콩·대만), 중국어 간체(중국 본토). 대만은 별도 로케일 없이 번체에 통합되며, 간체 지원은 언어 지원일 뿐 본토 네트워크 접근성을 보장하지 않는다(C14·R24).

| 항목 | 사양 |
|---|---|
| 언어 감지 | 브라우저 언어·IP 기반 자동 감지 + 수동 전환 |
| URL 구조 | 서브경로 방식 — /ko, /en, /ja, /zh-HK, /zh-CN |
| 통화 표시 | KRW 메인 표시 + 외화(USD/JPY/HKD/CNY — C17 enum 한정) 참고 환산 병기, 사용자 오버라이드 가능(§통화 정책) |
...
| 가격 (3인, 총액) | — | 1,000,000 KRW | 3,000,000 KRW |
| 가격 (4인, 총액) | — | 1,250,000 KRW | 3,750,000 KRW |
| 가격 (5인, 총액) | — | 1,500,000 KRW | 4,500,000 KRW |
| 소요 시간 | 2시간 | 2시간 | 3시간 |
| 인원 | 1~2명 | 1~5명 | 1~5명 |
...
| 보정본 | 미포함 | 미포함 | 미포함 |
| 뮤직비디오 | — | — | 포함 (외부 전문팀) |
| CD 앨범 (물리) | 포함 (당일 원본 현장 전달) | 포함 (당일 원본 현장 전달) | 포함 (당일 원본 현장 전달) |
| 헤어/메이크업 | 미포함 | 미포함 | 미포함 |
| 통역·외국어 | 지원 | 지원 | 지원 |

> **Gold 명칭 주석 (C8)**: Gold는 구 "Personal Vocal"이 통합·개명된 패키지다. 기존 "Personal Vocal" URL은 /packages/gold로 301 리다이렉트한다.

> **체험 인원별 과금 정책 (C10 정정·확장)**: 체험 패키지(Gold·Diamond·Premium)는 **인당 +50%** 선형 공식으로 과금한다 — **총액 = 1인 정가 × (1 + 0.5 × (인원 − 1))**. 즉 2인=1.5배, 3인=2.0배, 4인=2.5배, 5인=3.0배. 예: Diamond 1인 500,000 → 2인 750,000 → 5인 1,500,000. 인건비(엔지니어·디렉터·통역)는 1팀이 전 인원을 함께 진행하므로 매출은 인원에 선형 증가하나 원가 증분은 작아, 인원이 많을수록 마진율이 상승한다. 체험 최대 5인. 구 정책 "2인 1인당 반값(Diamond 250,000/인)"은 폐기. (1·2인은 v0.2 가격 모델 검증, 3~5인은 동일 공식 선형 확장 — M2 카탈로그 슬라이스에서 확정.)
...
| 소요 시간 | 2시간 | 2시간 | 2시간 |
| 인원 | 3~15명 | 10~15명 | 5~15명 |
| 처리 흐름 | B2B 견적·인보이스 (2026-07-17 정정 — 구 "일반 결제" 폐기, 웹 셀프예약 아님) | B2B 견적·인보이스 | B2B 견적·인보이스 |
| 예약 모델 | 전세(1팀 룸 점유, 합석 불가) | 전세(1팀 룸 점유, 합석 불가) | 전세(1팀 룸 점유, 합석 불가) |

> **단체 과금·노출 정책 (C11 정정)**: 단체 3종은 순수 인원당 정액 과금이다(총액 = 1인당 정액 × 참가 인원). 인건비는 회당 1팀 고정이므로 참가 인원이 많을수록 마진율이 상승한다. 노출은 분리된다 — 다국어(영·일·중간·중번) 사이트에는 **K-Pop Making Class만**, 한국어 사이트에는 **꿈길·워크샵만** 노출한다(K-Pop Making Class는 전 언어 노출이므로 한국어 사이트에도 표시). 꿈길 최대 인원은 20명에서 15명으로 정정. 단체 3종(K-Pop Making Class·꿈길·워크샵)은 모두 자체(D2C) 웹사이트 전용 판매이며, OTA(KLOOK·KKday·Airbnb)에는 노출·판매하지 않는다(영구 정책 — F 종결).
...
| K-Pop Making Class (전 언어, **B2B 문의·견적 — 웹 셀프예약 아님**) | 2시간 / 2슬롯 (평일만) | 10–12, 14–16 |

> **단체(Making Class) 슬롯 예외**: Making Class는 평일 10:00·14:00 2슬롯만 운영한다. **예약 경로는 웹 셀프예약이 아니라 B2B 문의 → 어드민 견적 전환(§5.8-A③, `booking_flow='b2b_quote'` — 2026-07-17 확정)이며, 슬롯 점유는 어드민 등록으로 반영된다.** **사유: 주말 및 그 외 시간대는 고단가 개인 패키지(Gold·Diamond·Premium) 우선 배정 정책에 따라 단체에 미개방.** Making은 전세(1팀 룸 점유)이나 룸 종일 독점이 아니며, 같은 평일 같은 룸의 비점유 시간대(예: 12–14, 16–22)는 개인 예약에 개방한다. 멀티룸 전환 시 룸별로 이 정책을 재설정할 수 있다.
...
**패키지 간 충돌 처리**: 같은 룸·같은 날의 시간 점유 매트릭스로 계산한다. Premium(3시간)·1Pro(3.5시간) 예약은 해당 시간대와 겹치는 모든 Gold/Diamond/1Hour 슬롯을 자동 마감한다. K-Pop Making Class(2시간, 10–12·14–16) 예약은 같은 룸·같은 시간대의 개인 슬롯과 상호 배타로 처리하며(Making이 점유한 칸의 개인 슬롯 자동 마감, 역방향 동일), 비점유 시간대는 개인 예약에 개방한다. 한국어 사이트와 외국어 사이트는 분리 표시되지만, 동일 룸의 시간 점유는 통합 관리된다.
...
**동시성 처리 (락 모델 — 2중 안전망, C19):** 더블부킹은 ① Redis 굵은 락(직렬화) + ② DB exclusion 제약(물리적 최종 차단)으로 막는다.

**① Redis 락 — 룸+날 단위.** 결제 시도 시 Redis 분산락으로 해당 **룸·날 전체**를 15분 임시 홀드한다(키: `slot_lock:{room_id}:{date}`, TTL 900초). 흐름: 락 획득 → DB에서 `(room_id, date, [start_time, end_time))` overlap 검사 → 겹침 없으면 예약 확정 → 락 해제. 결제 성공 시 슬롯 확정, 실패·이탈·타임아웃 시 락 자동 만료.
- **변경 사유 (기존 키 스킴 교정):** 기존 키 `slot_lock:{room_id}:{date}:{start_time}`(단일 시작시각)는 **길이가 다른 패키지의 overlap을 막지 못한다** — 예: Premium(3h, 10–13)과 Gold(2h, 12–14)는 시작시각이 달라 별개 락이 잡히지만 시간이 겹쳐 동시 결제 시 더블부킹이 가능했다. 키에서 `start_time`을 제거해 한 룸·한 날의 예약 처리를 **직렬화**하면 overlap 검사가 항상 단일 임계영역 안에서 일어나 이 결함이 사라진다.
- **동시성 trade-off:** room+date 직렬화로 동시 예약 처리량이 낮아지나, 런칭 룸 1개·홀드 15분·일일 슬롯 수 적음(룸당 ≤6) → 실질 부하 문제 없음. **정확성 우선**의 의도적 거래다(멀티룸 전환 시 락은 룸별이라 룸 수만큼 병렬).

**② DB exclusion 제약 — 최종 안전망.** Postgres `EXCLUDE USING gist`로 `(room_id =, tsrange[start_time, end_time) &&)` 겹침을 **DB가 거부**한다(`btree_gist` 확장 필요). 애플리케이션 로직(①)에 버그가 있어도 DB가 물리적으로 겹친 예약을 거부 = §4 "코드만 믿지 않는다". 기존 **"가용성 SSOT = 우리 DB"** 원칙과 일관(DB가 최종 진실). ⚠️ 이 제약·`btree_gist` 도입은 §4·§7-A.5 위험 구역이라 **별도 마이그레이션 슬라이스**로 구현한다(여기서는 계약만, 구현 아님).

**슬롯 시각 저장 규약:** `date`/`start_time`/`end_time`은 KST 벽시계값으로 저장한다(컬럼 타입 `date`/`time without time zone`, timezone-naive). `start_time`/`end_time`은 영업시간(10:00–22:00 KST)과 패키지별 시작시각 상수에서 정수 시·분 산술로 자동 계산하며, JS Date/UTC epoch를 경유하지 않는다. EXCLUDE 제약(`tsrange`)이 이 naive 벽시계 가정에 의존하므로, 절대시각·`timestamptz`·`Date.getTime()` 기반 변환을 슬롯 시각에 사용하는 것을 금지한다.
...
**D. 시스템 더블부킹 경합 환불** — 동시 결제 경합에서 슬롯을 확보하지 못한 예약(PG 캡처는 성공했으나 confirmBooking이 DB exclusion 위반(23P01)으로 차단되거나, 비동기 정산 webhook 재검증 시점에 슬롯이 이미 점유된 경우). 100% 자동 환불, PG 수수료 공제 없음(사업자 흡수). 결제 페이지~PG 구간에 룸 홀드를 두지 않는 설계상 선택(§5.5 결제 캡처 모델)이 원인이므로 고객 귀책이 아니며, 세션이 발생하지 않았으므로 다운로드 50% 상한도 적용되지 않는다. 환불 트리거는 시스템(어드민 개입 없음, `adminUserId=null`)이다. MVP는 환불만 처리하며 보상 쿠폰은 출시 후 재방문 모듈(Stage 6) 정합 시 검토한다. 기록 메커니즘: 동기(23P01) 경로는 confirmBooking 트랜잭션 롤백으로 결제 레코드가 남지 않으므로 별도 Refund 레코드 없이 PG 환불 실행 + audit_log 기록으로 처리하고, `concurrent_booking_lost` Refund 레코드 생성은 결제 레코드가 선존재하는 비동기 정산 webhook 재검증 실패 경로에 한정한다.

PG 수수료 공제 여부는 결제 직전 약관 동의 화면에 명시한다. 어드민에서 정책 수정이 가능하되, 결제 시점의 정책을 예약 레코드에 스냅샷으로 저장하여 소급 적용을 방지한다. 다운로드가 1회 이상 발생한 예약은 환불 최대 50%로 제한한다(사업자 귀책·천재지변·시스템 더블부킹 제외).
...
체험·대여·단체 카탈로그는 완전히 분리된 진입점으로 운영하며, 검색·필터·정렬도 카테고리 단위로 격리한다. 패키지의 `languages_available` 필드(예: `['ko']` 또는 `['ko','en','ja','zh-HK','zh-CN']`)로 언어별 사이트 카탈로그 노출을 자동 필터링한다. 1Hour·1Pro·꿈길·워크샵은 `['ko']`만 가지므로 외국어 사이트 빌드 시 자동 제외된다.
...
**표시통화 선택 체인 (2026-07-02 확정):** 로케일 기본값(C14) → 헤더 글로벌 통화 드롭다운(언어 셀렉터 옆)으로 사용자 오버라이드 → 선택값은 기능성 쿠키(TTL 30일)에만 저장. **DB에 통화 선호 필드를 두지 않는다** — payments의 `display_currency`·`exchange_rate_at_payment`(C17)는 선호 저장이 아니라 **결제 시점 스냅샷**으로 별개이며 유지한다. 외화 병기 범위 = 브라우징·패키지·견적 단계까지. **결제확인·결제 직전 화면은 KRW 단독 표시**로 하고 외화 병기를 제거하며, "결제는 KRW로 청구되며 실제 청구액은 카드사 환율에 따라 달라질 수 있음" 고지문을 표기한다(결제 직전 동의 화면의 약관 명시와 연동).
...
동시예약 방지는 §5.3 동시성 처리(C19)의 2중 안전망에 위임한다 — ① confirmBooking write 순간의 짧은 룸·날 직렬화 락(`slot_lock:{room_id}:{date}`, 임계영역 내 overlap 검사→확정→즉시 해제; 15분 TTL은 콜백 비정상 종료 시 자동 해제용 안전망일 뿐 결제 세션을 덮는 홀드가 아니다), ② Postgres `EXCLUDE USING gist` 제약이 겹침을 물리적으로 최종 거부. 결제 페이지 진입~PG redirect 동안 별도의 룸 점유 홀드는 두지 않는다(룸 1개·저트래픽, 정확성 우선 — §5.3 근거). 따라서 동일 슬롯에 두 고객이 동시에 PG까지 진행할 수 있으며, 먼저 캡처 성공해 confirmBooking을 통과한 예약이 슬롯을 확정하고, 늦은 쪽은 DB exclusion 위반으로 차단되어 자동 환불된다.

### 결제 캡처 모델 (동기 확정)

MVP는 **즉시승인(동기 캡처) 수단만 슬롯을 확정한다** — 신용/체크카드·네이버페이·카카오페이·PayPal. PG 캡처 성공 직후 confirmBooking이 Booking(`confirmed`)과 Payment(`paid`)를 **단일 트랜잭션**으로 기록하며, `confirmed` 전환이 예약 확정 자동화 체인(§5.8-A②)을 발화시킨다. 계좌이체(가상계좌)·휴대폰결제 등 비동기 정산 수단은 정산 webhook 수신 시점에 슬롯 overlap을 재검증한 뒤 확정하며, 재검증 실패 시 자동 환불한다(슬롯 점유는 `paid|confirmed|completed` 상태만 — 비동기 pending은 점유하지 않으므로 webhook 수신까지 슬롯이 열려 있다). 수단별 동기/비동기 분류는 KG이니시스 연동 시 확정한다.
...
PDF 생성 라이브러리: `@react-pdf/renderer` (Next.js 환경, 다국어 폰트 임베딩 지원). 적용 범위: 영수증 PDF, Dispute Evidence Pack, 동의서 PDF.
...
├─ display_currency(enum DisplayCurrency: KRW|USD|JPY|HKD|CNY — C17), display_amount, exchange_rate_at_payment  ※ 결제 시점 스냅샷 — 통화 선호 저장 아님(선호는 쿠키만, §통화 정책)
...
dispute_evidences ├─ booking_id, generated_at, pdf_storage_key, generated_by
```
...
- **CD 앨범은 물리 제공물(다운로드 딜리버러블 아님).** Gold·Diamond·Premium 체험에 포함(대여·단체 제외). 엔지니어가 **당일 세션 원본 테이크를 현장에서 CD-R로 구워 세션 종료 시 즉시 전달**한다(국제 배송·주소 수집 없음). 최종 믹스·마스터는 별개로 매직링크 디지털 전달(위 매트릭스). 스키마상 새 `DeliverableType`를 만들지 않고, `packages.cd_included` 게이트(Gold/Dia/Premium) + 체크인 레코드의 이행 필드(`checkins.cd_handed_over_at`·`cd_handed_over_by`, `checked_in_at`/`by` 패턴 미러)로만 추적하며, 배송 상태·주소 스키마는 두지 않는다.
...
음원·사진은 Railway worker(트랜스코딩 서비스)에서 처리한다(Premium MV는 외부팀 납품으로 트랜스코딩 없음 — 인프라 피벗 2026-07-18, 구 Cloud Run Jobs 대체).

| 출력 | 사양 | 처리 |
|---|---|---|
| WAV 마스터 | 16bit 44.1kHz Stereo | worker FFmpeg (녹음 원본 24bit/48kHz → 다운컨버트) |
| MP3 | 320kbps CBR Stereo | worker FFmpeg |
| 사진 | JPG 원본 무보정, EXIF 보존 | worker Sharp (썸네일 WebP 400px 자동 생성) |
...
|---|---|
| 매직링크 유효기간 | 60일 |
| 스토리지 파일 보관 | 게스트 3개월 / 회원 12개월 |
| 만료 링크 재발급 | 어드민에서 가능 (고객 CS 요청 시) |
| 만료 사전 안내 | 파일 보관 종료 30일 전 이메일 자동 발송 |

> **매직링크 토큰 보안 (C15 정정):** 토큰을 raw UUID로 PK 겸 노출하던 초기 설계를 폐기한다. PK는 cuid()(내부 참조용), URL에 들어가는 토큰(`/download/[token]`)은 별도 crypto 난수로 생성하고 고객 이메일로만 발송하며, **DB에는 토큰의 SHA-256 해시만 저장**한다(원본 미저장). 검증 시 들어온 토큰을 해시해 비교. DB 유출 시에도 활성 다운로드 링크가 노출되지 않는다(Stage 1 비밀번호 재설정·이메일 검증 토큰과 동일 원칙). 재발급 시 구 토큰은 status=revoked로 무효화. download_logs는 토큰 원본이 아니라 magic_link PK(cuid)로 FK 연결.

서명 URL 기반(Supabase Storage 비공개 버킷, TTL 10분, 페이지 새로고침 시 자동 갱신 — 인프라 피벗 2026-07-18). 다운로드 시 IP·시각·UA·파일명을 `download_logs`에 기록한다 — 이는 앱 로그가 아니라 **DB 감사 레코드**다(목적 제한: 어뷰즈 감지·분쟁 증거 팩 §5.5. 보존 3년, 접근 어드민 RBAC 한정. CLAUDE.md §3.6과의 관계는 해당 조항의 2026-07-18 명확화 참조). 동일 IP에서 비정상 다운로드 빈도(5분 내 50회) 감지 시 자동 일시 차단(어드민 알림 자동발화는 Stage 7 알림 자동화 — 이 스테이지는 차단까지 구현하고 알림 배선은 defer, `app/api/download/file` 참조).
...
├─ storage_key, file_size_bytes, uploaded_by, uploaded_at, released_at
├─ supersedes_id, retention_until

magic_links ├─ id(cuid), token_hash(SHA-256 — 원본 토큰은 crypto 난수, 메일/URL로만 발송·DB 미저장, C15), booking_id, generated_at, expires_at(+60일), last_accessed_at, access_count, status('active'|'expired'|'revoked')
...
동의 기록은 append-only 테이블로 저장한다(UPDATE/DELETE를 DB 트리거로 차단). 철회는 같은 타입의 새 row를 `consented=false`로 삽입한다. 약관 개정 시 새 버전을 발행하고 동의 시점의 약관 버전 + SHA-256 해시를 함께 저장한다. 결제 완료 시 동의서 PDF를 자동 생성하여 비공개 버킷(service-role 전용 쓰기)에 저장하고, **PDF의 SHA-256 해시를 append-only 동의 레코드에 기록**해 무결성을 증명한다(접근은 audit_log). 구 Bucket Lock(WORM)은 인프라 피벗(D1)으로 대체 — 법무 감수 시 분쟁 증거 충분성 확인 단서.
...
```
consents (append-only, UPDATE/DELETE 금지 — DB 트리거로 강제)
├─ id(UUID @db.Uuid), booking_id, user_id
...
booking_participants (미성년 검증 SSOT — §3.4)
├─ id(cuid), booking_id(FK → bookings)
├─ date_of_birth(🔒 PII, Date) — GDPR 삭제·익명화 시 이 컬럼만 처리
├─ is_minor(판정 시점 스냅샷, 기준 시점은 5.7 — 예약 시점)
├─ guardian_consent_id(FK → consents, nullable — 미성년일 때만)
├─ created_at
...
song_licenses ├─ id(cuid), song_id, type('recording'|'mr_distribution'|'lyrics'), verified, evidence_storage_key(비공개 버킷), notes, verified_at, verified_by  @@unique(song_id, type)  # 종류별 라이선스 (C16, R2)
```
## 5.8 어드민 전체 구조
...
| Manager | 환불·권한 부여·약관 발행 제외 전 권한 (단, §5.8-A 게이트 3종은 Super Admin 전용 — 아래 주석) |
...
> **설정 권한 충돌 해소 (OPEN DECISION #2, v0.2):** §5.8의 "Manager = 설정 가능"과 §5.8-A의 "특정 게이트는 Super Admin 전용"이 충돌하던 것을 fail-safe(더 제한적 규칙 우선)로 해소한다. 일반 설정(`settings:manage` — 영업시간·룸 토글 등)은 Manager 이상 가능하나, 아래 **게이트 3종은 Super Admin 전용 + 재인증** 필수다: ① `gate:mr_predelivery`(연습용 MR 사전 배포 — R2 라이선스 게이트), ② `gate:license_display`(라이선스 표기 토글 — R2), ③ `seo:custom_script`(SEO Code Injection 자유 스크립트 — XSS 방어, §5.8-A①). 사유: 세 게이트는 모두 위험 구역(저작권 침해·XSS)이라 Manager 계정 실수·탈취 시 법적·보안 사고로 직결되므로 격리한다.

**민감 액션 (재인증 강제)** — 환불 처리, 권한 부여·회수, 계정 삭제·비활성화, 대량 데이터 export(100건 이상), 약관·처리방침 신규 버전 발행, 동의 PDF 버킷 보존·정책 변경. 비밀번호 재입력 + TOTP 재확인을 강제한다.
...
**감사 로그 (audit_logs)** — append-only(UPDATE/DELETE 차단). 기록 대상: 로그인·로그아웃·로그인 실패, 환불 처리, 권한 변경, 동의 조회·export, 개인정보(PII) 조회, 예약 수정·삭제, 콘텐츠 업로드·다운로드, 데이터 export, 매직링크 재발급, 블랙아웃 생성, 가격 변경, 약관 발행, 민감 액션 재인증. 보관 3년 이상(전자상거래법 기준 5년 권장), PostgreSQL 월별 파티셔닝 + 월간 스토리지 아카이브(비공개 버킷). Super Admin은 조회만 가능.
...
│  pdf_storage_key, issued_by, issued_at, accepted_at
```
...
mv_deliveries ├─ id, booking_id, mv_team_id, status, expected_delivery_date, actual_delivery_date,
              ...
              │  main_file_storage_key, short_file_storage_key, uploaded_by, uploaded_at, sent_to_customer_at
...
다운로드 페이지(매직링크 진입)에 인라인 후기 양식을 둔다. 별점 1~5(필수), 좋았던 점 태그 1~5개 선택(보컬 디렉팅·결과물 음원 품질·사진/영상·시설·직원 친절도·가성비·곡 다양성), 자유 텍스트(최대 500자), 작성자 표시명(전체 이름에서 서버가 자동 생성한 이니셜 마스킹으로 **고정** — 사용자가 표시명을 직접 지정할 수 없다. 전체 이름은 `author_name_snapshot`(PII)에만 보관하고 노출하지 않는다), 이용 서비스·일자 자동 표시. 후기 작성은 선택이며, 다운로드는 조건 없이 가능하다.
...
**제출 정책** — 후기는 **예약(booking) 1건당 1행**이며, `reviews.booking_id`에 유니크 제약을 둔다. 단체·워크샵 세션에서도 예약 대표 1인이 1건만 작성하며, 참가자별 개별 후기는 지원하지 않는다. `booking_participants` 모델이 별도로 존재하지만 후기와는 연결하지 않는다 — 참가자별 후기가 필요해지면 유니크 제약을 해제하지 말고 별도 모델로 설계할 것(제약 단위는 인원이 아니라 예약이다). 동일 예약의 재제출은 409로 종결한다. 제출 rate limit은 §6.2 열거를 따른다.

**IP 취급** — `reviews.ip`는 어뷰즈 탐지와 결제 분쟁 대응(§4 Dispute Evidence 패키지) 목적으로만 보관한다. 공개 후기 조회 API 응답에 포함하지 않으며, 어드민 후기 상세 화면 외 어디에도 노출하지 않는다. 보존기간은 후기 본문과 동일하다.

**수정·삭제** — 현행은 제출 후 수정·삭제 경로가 없다(재제출 409). 작성자 본인의 수정·삭제권은 개인정보 자기결정권 측면에서 향후 제공하며([R] 트랙), 이는 표시광고법이 금지하는 **사업자에 의한 선별 삭제**와는 별개 사안이다. 사업자 삭제 제한은 위 노출 정책을 그대로 유지한다.
...
| 항목 | 정책 |
|---|---|
| 할인율 | 10% 정률 (인원 과금 총액 기준) |
| 할인 상한 | ₩50,000 (친구 초대와 동일) |
| 자격 | 로그인 회원 + 직전 완료·결제완료(환불 제외) 예약 1건 이상. 게스트 미적용(→ 회원 전환 유도 지점, §5.10) |
| 적용 패키지 | 체험만 (Gold·Diamond·Premium). 대여·단체 제외 |
| 적용 방식 | 자격 충족 시 체크아웃 자동 적용 (코드 입력 불필요) |
| 유효기간 | 무기한 |
| 환불 영향 | 할인받은 예약을 환불해도 원 완료 이력은 유지 → 이후 자격에 영향 없음 |

친구 초대 invitee 할인과 동일하게 **보유 쿠폰 인스트루먼트가 아니라 체크아웃 인라인 할인**으로 적용하며, 적용액은 결제 시점 스냅샷(§3.2)으로 예약에 동결한다(`bookings.returning_discount_krw`). 자격 판정은 회원의 과거 예약 이력(완료·결제완료 1건 이상) 조회로 수행하고, 패키지 노출은 `packages.returning_discount_eligible` 게이트로 제어한다.

### 프로모션 코드

정률(기본 10%)·정액 지원, 할인율 범위 1~50%, 15% 초과 발급 시 Manager·Super Admin 승인 강제(재인증). 사용 방식(1회용/다회/1인 1회), 만료일 필수, 적용 패키지·언어 토글, 최소 결제액·전체 한도 설정, 발급 채널 태그 필수(효과 측정). **고객 셀프-획득 할인 채널은 재방문 회원·친구 초대 두 가지로 한정한다(오너 확정 2026-07-17).** 프로모션 코드는 어드민이 발급하는 마케팅 수단으로 유지되나, 고객이 스스로 획득하는 할인 채널로 취급하지 않는다. 모든 할인은 상호 배타적이다 — 재방문 회원·친구 초대·프로모션 코드 중 최대 1개만 적용하며, 고객에게 가장 유리한 금액을 자동 선택한다(동률이면 재방문 회원 할인 우선 — 보유 쿠폰 미소진). 시스템 자동 검증. 프로모션 코드는 자체(D2C) 채널 결제에만 적용되며, OTA(KLOOK·KKday·Airbnb) 채널 예약에는 적용하지 않는다(어드민 수기 등록 결제 시에도 프로모션 할인 차단, 시스템 자동 검증).
...
프로필(이메일·이름·연락처·국적·여권명·비밀번호·연결 소셜 계정·선호 언어) / 내 예약(예정·완료·취소) / 콘텐츠 라이브러리(매직링크 통합·카테고리 필터·보관 만료 D-day·일괄 다운로드) / 친구 초대(코드·공유 링크·초대 현황·받은 쿠폰) / 후기(작성 내역 조회 — 수정·삭제 기능은 [R] 트랙 예정, 현행 미구현) / 동의·개인정보(동의 이력·마케팅 동의 채널별 토글·데이터 다운로드·계정 삭제 요청) / 알림 설정 / 로그아웃·전체 기기 로그아웃.
...
**기본 보안 (상시)** — HTTPS 강제(TLS 1.3), HSTS 1년, HTTP→HTTPS 301. 모든 입력 검증(Zod), SQL 인젝션·XSS·CSRF 방어. 비밀번호 bcrypt cost 12, 어드민 2FA 필수. 환경변수는 GCP Secret Manager(Git 금지). Rate limit: 로그인 5회/5분, API 100req/분/IP, 결제 3회/10분/사용자, 후기 제출 20회/5분/IP(※ 코드 현행 5회/5분 — 본 수치로 조정 예정, 코드 슬라이스 별도). CSP 헤더, X-Frame-Options DENY, Referrer-Policy strict-origin.
...
메인 키워드 예시: 영어 "k-pop recording experience seoul", "kpop studio seoul tourist"; 일본어 "韓国 K-POP 録音体験"; 중국어 번체(대만) "首爾 K-POP 錄音體驗"; 중국어 번체(홍콩) "首爾 K-POP 錄音體驗".
...
### SEO 구현 워크스트림 분해 (2b-SEO 슬라이스 계약)

곡 상세 SEO는 단일 슬라이스가 아니라 **4개 슬라이스**로 분해한다. 곡 상세 렌더(2b-2b)는 사이트 전체 SEO 인프라와 스키마 확장에 종속되므로, 인프라를 곡에 묶지 않고 분리해 다른 콘텐츠 타입(패키지 카탈로그 등)이 동일 인프라를 재사용하게 한다. 이 절은 **구현이 아니라 계약**이다(코드/스키마 변경 없음).

**슬라이스 경계·선행조건**

| 슬라이스 | 범위 | 선행조건 | 비고 |
|---|---|---|---|
| **2b-SEO-infra** | `robots.ts`·`sitemap.ts` 골격, hreflang(`alternates.languages`) 골격, PageSeo/PageSchema 소비 **추상화 인터페이스(필드 계약만, 렌더 구현 아님)** | 없음 | 사이트 전체 인프라, 곡 비종속. 패키지 등 타 콘텐츠 SEO가 동일 인프라 재사용 |
| **2b-SEO-migration** | 곡 `slug` 추가(곡↔URL·PageSeo 연결), per-locale `description`(SongTranslation 확장), MusicRecording JSON-LD 템플릿(SchemaTemplate 8번째) | 없음(infra와 병행 가능) | 워크플로: PRD 문서화 먼저 → Antigravity 마이그레이션 구현(§7-A.5 표준 절차, 파괴적 작업 검토 후) |
| **2b-2b** | `generateMetadata`, JSON-LD 출력, `sitemap.ts` 곡 URL 채우기 | **2b-SEO-infra + 2b-SEO-migration 완료** | 소비할 모델·인터페이스 안정 후 착수 |
| **2b-SEO-ai** | AI 콘텐츠 생성·검수 파이프라인 | **콘텐츠 전략 결정(현재 미결 = 블로킹)** | 전략 확정 전 착수 금지. v1.1+ AI 보조(§6.3 API 비용 통제)와 정합 |
...
- slugify 구현: 현행 repo에 slugify 라이브러리·함수 **없음**(`packages.slug`는 수작업 입력). → **확정(Aiden): `@sindresorhus/slugify`**, 옵션 `{ lowercase: true, separator: '-', decamelize: false }`. ASCII·diacritic 정규화는 처리하나 **한글·CJK는 transliterate 안 함 → 빈 slug 발생((3) Phase 2 처리)**. immutable이라 이 lib·옵션 고정, 이후 변경 금지.
...
**(3) 비ASCII / 빈 slug 예외 — ⏸ Phase 2 보류 (실 데이터 적재 후 결정, Aiden):** canonical은 NOT NULL이나 ASCII 보장 없음. slugify 결과가 빈 값/`-`만일 때의 처리. **2-페이즈 분할 채택**(아래 게이트) — 실 ~2,500곡 적재 후 **비ASCII 규모를 보고** 결정한다(규모 미지수 상태에서 immutable 값 확정 회피). 후보(보존):
...
**(4) 유효성 제약:** slug **UNIQUE**(전 구간). **NOT NULL 전면 강제는 Phase 2**((3) 확정·전체 backfill 후) — Phase 1은 비ASCII 보류분 때문에 nullable.
- 길이 상한: **확정 최대 80자**(Aiden) — slugify 후 80자 초과 시 절단 후 (2)의 충돌 suffix 부여(절단이 충돌을 늘려도 suffix가 흡수).

**선행 게이트 — 2-페이즈 분할 (Aiden 채택):**
- **Phase 1 (착수 가능 — 차단 해제):** slug 컬럼 신설 + 생성 로직(확정 `@sindresorhus/slugify` · 80자 · `song.id` 오름차순 결정적 충돌) + **ASCII 파생 가능 행** backfill. slug **nullable**(비ASCII 보류분은 NULL → 해당 곡 sitemap·상세 URL 미생성). (1)·(4) 확정으로 차단 해제됨.
- **Phase 2 (게이트):** 실 ~2,500곡 적재 후 (3) 비ASCII fallback 확정 → 잔여 행 backfill → **NOT NULL 강제**. **(3) 미확정 시 Phase 2 착수 금지**(immutable = 되돌릴 수 없는 행 사고).

### 곡 MusicRecording JSON-LD 계약 (2b-2b-3 사전, Aiden 확정)

곡 상세(`/songs/[slug]`) JSON-LD의 타입·속성 매핑·템플릿 행 생성 방식을 본체 구현(2b-2b-3) 전에 계약으로 확정한다(이 절은 계약만 — 코드/스키마/데이터 변경 0). 사전 진단(읽기 전용, 2026-06-21)이 드러낸 공백 3건(속성 매핑 미계약·선택 근거 미기록·템플릿 행 생성 방식 미결)을 메우고, §6.3 본문(구조화 데이터 7종, Product+Offer)↔C18(8번째, MusicRecording) 긴장을 해소한다.

**결정 1 — 스키마 타입: MusicRecording 유지.**
- 곡 상세 페이지 JSON-LD 타입 = schema.org **MusicRecording**.
- **의미 보정:** `recordingOf`로 원곡(**MusicComposition**)을 중첩 참조한다. King Studio는 공식 음원 제공자가 아니라 고객의 녹음 체험 레퍼토리를 제공하므로, `recordingOf`로 "이 음원이 담는 원곡"을 가리켜 의미 정확도를 보완한다.
- **§6.3 본문↔C18 긴장 해소:** 곡 페이지=MusicRecording / 패키지(체험 상품) 페이지=Product+Offer는 **서로 다른 엔티티이며 충돌이 아니다**. §6.3 본문(기술 SEO·구조화 데이터 관리)의 7종은 **패키지·상품 페이지용**, MusicRecording(8번째)은 **곡 페이지 전용**이다. **Product/Offer는 곡 페이지에 부적합** — 곡 페이지엔 가격·`offers`가 없어 Product를 붙이면 의미 오류 + Google "Product without offers" 리치결과 위반 위험이 있으므로 곡 페이지에 쓰지 않는다.
- **참고(실익):** MusicRecording은 Google 리치결과 피처가 없다(SERP 스니펫 강화 없음). 실익은 엔티티/지식그래프 + GEO/AEO 인용 맥락이다.

**결정 2 — 속성 매핑 (Song 필드 → schema.org).** locale 해석은 §5.4 체인(요청→en→canonical)을 재사용한다.
- `name` ← `Song.title`(§5.4 해석값).
- `byArtist` ← `Song.artist`(§5.4 해석값). **표현:** 문자열 artist를 `{ "@type": "MusicGroup", "name": <artist> }` **최소 객체**로 감싼다(Person 아님 — K-POP 그룹 기본; `name`만 보유).
- `recordingOf` ← `{ "@type": "MusicComposition", "name": <원곡명> }`. 원곡명은 곡 title(원곡=수록곡 동일 명칭)로 채운다.
- `description` ← per-locale description이 **있으면 포함, 없으면 생략**(메타 description과 동일 출처·동일 optional 규칙 — 위 "메타 파생 규칙"과 일관).
- **데이터 없는 속성(`inAlbum`·`duration`·`datePublished` 등) = 출력 제외.** 빈/추측 값 출력 금지(빈 속성은 리치결과 품질 저하·허위 표시 위험).

**결정 3 — SchemaTemplate 데이터 행 생성 방식: seed 스크립트(멱등).**
- SchemaTemplate 행은 **seed 스크립트로 생성·관리**한다(어드민 화면 아님).
- **근거:** SchemaTemplate은 개발자가 정하는 **코드성 설정(거의 불변)**이며, 변경은 코드 리뷰·git 추적을 거쳐야 한다. 어드민 화면 부재 + 잘못된 입력이 **전 페이지 구조화 데이터를 깨는** 위험을 회피한다.
- **멱등:** 이미 존재하면 변경 없음 또는 업데이트(곡 slug backfill 멱등 패턴과 동일 원칙). 재실행 안전.
- **⚠️ 기존 7행도 현재 미생성 상태** — 이 seed 방식이 **8개 전체에 적용될 패턴**이다. 단 **이번 슬라이스(2b-2b-3)는 MusicRecording 1행만** 다루고, **나머지 7행 seed는 별도 슬라이스로 분리**한다(이번 범위 밖, **부채**).
- **역할 구분:** 운영자가 **곡별 SEO를 수동 덮어쓰는** 경우는 SchemaTemplate이 아니라 **기존 PageSeo override**(위 "메타 파생 규칙" 1번)가 담당한다. PageSeo 어드민 화면은 **별도·후속 슬라이스**.
...
> **⚠ 인프라 피벗 (2026-07-18, `Infra_Pivot_Decision_v1.md` 정본):** 단일 GCP → **Supabase(DB·Storage) + Railway(호스팅·worker)**. 이 장의 GCP 세부(§7.3 Cloud SQL·§7.4 Cloud Run/Cloud Build·§7.7 Cloud CDN·§7.13 비용 등)는 **피벗 미반영 구본**이며 충돌 시 피벗 결정 문서가 우선한다. 실 프로비저닝(플랜·리전·백업·비용 실측) 시 이 장을 재작성한다. Redis Upstash·Cloudflare Free·SaaS 항목(Resend·솔라피·Sentry 등)은 유효.

선택 원칙: ① 러닝커브 최소화, ② 한국 + 글로벌 동시 대응, ③ 운영비 ≤ ₩500K/월(MVP), ④ 풀스택 1명 유지보수 가능, ⑤ 관리형 서비스 우선(Supabase·Railway — 구 'GCP 단일 생태계' 대체).
...
# 파일·미디어 (Supabase Storage — 인프라 피벗 2026-07-18)
SUPABASE_URL
SUPABASE_SERVICE_ROLE_KEY        # 서버 전용 — 클라이언트 번들 유입 금지
SUPABASE_BUCKET_CONTENT / SUPABASE_BUCKET_CONSENT   # 비공개 버킷 — 서명 URL 전용
...
**포함** — 5개 언어(한·영·일·중번 zh-HK·중간 zh-CN), 8개 패키지(Gold·Diamond·Premium·1Hour·1Pro·Making Class·꿈길·워크샵), Premium MV(외부팀 사양 납품 + 어드민 업로드, 후속 자동 — C9), KG이니시스 + PayPal, KRW 단일 청구, 멀티룸 대비 슬롯 엔진(STUDIO A만 운영, Redis 락, 외부 채널 수기 등록), 자동 트랜스코딩 + 60일 매직링크, GDPR + 한국 PIPA + 미성년자 동의, 어드민 7개 역할·2FA·감사 로그·대시보드·알림, Channel Talk 5채널, 마케팅(후기·UGC 반자동·친구초대·프로모션·이메일 캠페인), 게스트 + 회원 + 마이페이지, Google·Apple 로그인, B2B 견적·인보이스·CSV·일괄 매직링크.
...
| 게이트 | 시점 | 통과 기준 | 미달 시 |
|---|---|---|---|
| Gate 1a | M3 종료 (2026-08-15) | 한·영 + Diamond 결제 처음~끝 작동 — **Mock PG 기준**(수수료 테이블·결제 스냅샷·슬롯 락·미성년 차단은 실 로직) | **진척 80% 미만 시 전체 일정 전면 재산정** |
| Gate 1b | 2026-09-15 목표 | **실 PG(KG이니시스·PayPal) 청구 처음~끝 작동** — sandbox 어댑터·webhook·영수증·환불은 심사와 병행 개발, 심사 통과 후 실 키 교체 + §4 위험 구역(webhook 위변조·중복) 사람 검증 | 심사 지연 시 **1b만 순연**(M4~M8 은 PG 독립 — 런칭 일정 불변) |
...
>
> **Gate 1 분리 개정 (2026-08-07, 오너 확정 B안)**: 구 Gate 1 을 1a(Mock)·1b(실 PG)로 분리.
> 근거 = R6 조기 판단의 실제 발동 — KG이니시스 가맹 심사(2~4주)·PayPal Business 가 8/7 시점
> 미신청이라 8/15 실결제 통과는 물리적으로 불가하나, 예약~결제 파이프라인(슬롯 락·스냅샷·
> 미성년 차단·e2e)은 Mock 게이트웨이 위에서 이미 작동한다. 실 PG 는 신청 서류(오너)가
> 크리티컬 패스이며 개발(sandbox)은 심사와 병행 가능 — 전면 순연(A) 대신 게이트 분리(B)로
> 런칭 일정을 보존한다. PG 신청 2건은 §9.8 대로 즉시 착수한다.
>
> **Gate 1a 통과 (2026-08-07 실측)**: 한(ko)·영(en) × Diamond, UI 처음~끝(패키지 선택 →
> 일정 → 옵션·동의 → Mock 결제) 각 1건 confirmed. 증거: 예약 2건(₩500,000 스냅샷 정가 일치·
> 동의 레코드 기록), 슬롯 소진 실동작(en 예약 후 ko 화면에서 해당 슬롯 "마감" 표시 — 색+텍스트
> 병기), KRW 단일 청구 고지 노출. e2e(booking-checkout: 미성년 차단·B2B 우회 차단) 별도 통과.
> 오너 운영 참고: KG이니시스 심사 신청은 다음 주 예정 — 심사 시점에 사이트에서 결제 흐름이
> 작동해야 하므로 이니시스 테스트 연동(sandbox) 슬라이스를 신청 전 완료 목표로 선행한다.
...
| R15 | 친구 초대 어뷰즈 (다중계정 쿠폰 반복 수령) | MVP는 약한 방어 — 발급 시점 Redis 중복·레이트리밋 체크 + 사후 어드민 검토(기존 데이터 기반). referrals에 IP·디바이스·BIN 미저장(§3.6 PII·결제정보 제약 준수). 출시 후 어뷰즈 실발생 관찰, 발생 시 v1.1에서 IP·디바이스 해시 기반 방어 추가(Stage 6 확정) |

## 10.4 Low 리스크 (모니터링 목록)

R12 환율 API Free 한도 초과, R15 2FA 디바이스 분실, R16 감사 로그 양 증가(파티셔닝 필요), R17 Cloud Run/CDN 피크 트래픽 청구, R18 Cloud Run Jobs 트랜스코딩 콜드스타트, R19 Apple Relay 이메일 도달, R20 OpenAI/Perplexity API 비용 폭주(하드 캡으로 통제), R21 친구 초대 어뷰즈, R22 외부 리뷰 채널 정책 변동, R23 재가입 7일 쿨다운 우회, R24 중국 본토 접근성·결제수단 갭(C14 — 간체 로케일은 지원하나 GFW 대응은 비목표이며, KG이니시스·PayPal이 UnionPay·Alipay·WeChat Pay를 미지원하여 본토 사용자 결제 완주율이 낮을 수 있음. 결제수단 확장은 M3 이후 별도 판정). 각 항목은 해당 장의 완화책으로 1차 대응하며, 분기 리스크 리뷰에서 등급 상향 여부를 점검한다.
...
| O3 | 가격 모델링 (경쟁사 리서치 + 단위원가) | ✅ 해소 (2026-06-30) | 단위마진·손익분기·단체마진 검증 완료, 마진 부호 전 패키지 + 확인. 세부 부채 A·B·C10·D·F·G 종결(F=Making 등 단체 OTA 미노출 결정, §5.2 반영 / G=OTA×프로모션 중복 봉쇄, §5.9). xlsx 모델 + stable-pricing 태그 참조 |
...
1. **PRD v0.2 정합 검증 완료** — 본 문서 (C1–C17 정정 반영 완료. C14 보강 완료 — 표시통화 오버라이드 체인 확정(2026-07-02, §통화 정책)).
