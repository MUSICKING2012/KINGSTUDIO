...
서명 URL 기반(Supabase Storage 비공개 버킷, TTL 10분, 페이지 새로고침 시 자동 갱신 — 인프라 피벗 2026-07-18). 다운로드 시 IP·시각·UA·파일명을 `download_logs`에 기록한다 — 이는 앱 로그가 아니라 **DB 감사 레코드**다(목적 제한: 어뷰즈 감지·분쟁 증거 팩 §5.5. 보존 3년, 접근 어드민 RBAC 한정. CLAUDE.md §3.6과의 관계는 해당 조항의 2026-07-18 명확화 참조). 동일 IP에서 비정상 다운로드 빈도(5분 내 50회) 감지 시 자동 일시 차단 + 어드민 알림.
