...
| C16 | songs 라이선스·다국어 | license_verified 단일 boolean → license_scope enum(recording/mr_distribution/lyrics) 종류별. title/artist 단일 String → Json 다국어. s3_key → storage_key (§5.7) |
| C17 | 결제 표시 통화 enum | payments.display_currency 자유 String → enum DisplayCurrency(KRW/USD/JPY/CNY/EUR…). 결제 위험구역, 통화 코드 불일치 방지 (§5.5) |
...
├─ display_currency(enum DisplayCurrency: KRW|USD|JPY|CNY|EUR|...), display_amount, exchange_rate_at_payment
...
songs ├─ id, title(Json 다국어), artist(Json 다국어), beginner_curation, is_active, license_scope(enum LicenseScope: recording|mr_distribution|lyrics), license_evidence_storage_key, license_notes, license_verified_at
```
## 5.8 어드민 전체 구조
