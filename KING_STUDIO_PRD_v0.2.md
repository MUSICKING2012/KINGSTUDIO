...
결제 페이지 진입 시 Redis에 **룸·날 단위** 15분 임시 락(키 `slot_lock:{room_id}:{date}`, TTL 900초). 결제 성공 시 락 해제 + DB 예약 확정, 실패·이탈·타임아웃 시 락 자동 만료. 동일 룸·날에 락이 이미 존재하면 다른 고객은 그 룸·날 예약 진입 불가, "잠시 후 다시 시도" 안내. 락 모델 상세·변경 근거·DB exclusion 최종 안전망은 §5.3 동시성 처리(C19) 참조.
