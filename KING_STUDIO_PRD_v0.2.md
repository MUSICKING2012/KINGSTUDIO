...
> **이 문서에 대하여**
> 본 문서는 v0.1·v0.2 단계의 다회 논의에서 합의된 모든 결정을 단일 문서로 통합하고, 장 간 교차 참조 충돌 18건(C1–C18)을 정정한 정합 검증본이다. "완결"은 설계 합의가 끝났음을 의미하며, 개발 착수 전 가격 모델링(미해결 이슈 O3) 완료가 선행되어야 한다.
...
| C15 | 매직링크 토큰 | raw UUID PK → **cuid PK + SHA-256 해시(`token_hash`)**. 원본 토큰은 crypto 난수로 메일/URL로만 발송·DB 미저장. download_logs는 `magic_link_id`(cuid)로 FK. Stage 1 토큰 원칙 일관 (v0.2 Stage 4 확정) |
...
| C18 | 회원 삭제 상태 | `users.status`에서 `deleted` 값 **제거** — 삭제 판정은 `deletedAt IS NOT NULL` **단일 기준**(SSOT, 이중 인코딩 제거). status는 `active`\|`inactive`만. 스키마를 §5.10 본문에 정합 (v0.2 Stage 4 확정) |
...
dispute_evidences ├─ booking_id, generated_at, pdf_storage_key(GCS — C13), generated_by
```
...
├─ storage_key(GCS — C13), file_size_bytes(BigInt), uploaded_by, uploaded_at, released_at
...
│  pdf_storage_key(GCS — C13), issued_by, issued_at, accepted_at
```
...
mv_deliveries ├─ id, booking_id, mv_team_id, status, expected_delivery_date, actual_delivery_date,
              ...
              │  main_file_storage_key, short_file_storage_key (GCS — C13), uploaded_by, uploaded_at, sent_to_customer_at
...
| 어뷰즈 차단 | 동일 IP·디바이스 핑거프린트·카드 BIN 셀프 초대 차단. **MVP는 약한 방어**(Upstash Ratelimit + 사후 검토; 어뷰즈 신호 IP·디바이스·BIN은 PII 미저장 — 런타임 비교만), **어뷰즈 발생 시 v1.1에서 해시 기반 방어 강화**(R21) |
...
users ├─ id(cuid), email(unique, 앱 단일지점 lowercase 정규화),
      │  emailVerified(DateTime? — null=미인증, Auth.js 표준 단일화, C12), passwordHash(nullable), name, phone, nationality,
      │  passportName, preferredLanguage, status(active|inactive), deletedAt
      │  # 삭제 판정 단일 기준 = deletedAt IS NOT NULL (status에 deleted 없음)
      │  # 미인증 = emailVerified IS NULL, 7일 후 inactive
...
R12 환율 API Free 한도 초과, R15 2FA 디바이스 분실, R16 감사 로그 양 증가(파티셔닝 필요), R17 Cloud Run/CDN 피크 트래픽 청구, R18 Cloud Run Jobs 트랜스코딩 콜드스타트, R19 Apple Relay 이메일 도달, R20 OpenAI/Perplexity API 비용 폭주(하드 캡으로 통제), R21 친구 초대 어뷰즈(MVP 약방어=Ratelimit+사후검토·신호 PII 미저장 → 발생 시 v1.1 해시 방어), R22 외부 리뷰 채널 정책 변동, R23 재가입 7일 쿨다운 우회, R24 중국 본토(간체) 미지원(C14 — 설계상 제외, 대만·홍콩 번체 타겟). 각 항목은 해당 장의 완화책으로 1차 대응하며, 분기 리스크 리뷰에서 등급 상향 여부를 점검한다.
...
1. **PRD v0.2 정합 검증 완료** — 본 문서 (C1–C18 정정 반영 완료).
