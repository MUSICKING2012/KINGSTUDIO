...
| C14 | 로케일·표시통화 | 중국어 2종 모두 **번체** 확정 — `zh-TW`(대만·TWD)·`zh-HK`(홍콩·HKD). 간체(`zh-Hans`·본토·CNY) 미지원 → GFW 대응 불필요. **기본** 표시통화 ko=KRW/en=USD/ja=JPY/zh-TW=TWD/zh-HK=HKD — 사용자 오버라이드 가능(§통화 정책, 2026-07-02 확정) (v0.2 Stage 3 확정) |
...
런칭 시점 5개 언어를 모두 지원한다: 한국어, 영어, 일본어, 중국어 번체(대만), 중국어 번체(홍콩). 중국어 2종은 모두 번체이며, 간체(중국 본토)는 미지원한다(C14).

| 항목 | 사양 |
|---|---|
| 언어 감지 | 브라우저 언어·IP 기반 자동 감지 + 수동 전환 |
| URL 구조 | 서브경로 방식 — /ko, /en, /ja, /zh-TW, /zh-HK |
| 통화 표시 | KRW 메인 표시 + 외화(USD/JPY/TWD/HKD — C17 enum 한정) 참고 환산 병기, 사용자 오버라이드 가능(§통화 정책) |
...
체험·대여·단체 카탈로그는 완전히 분리된 진입점으로 운영하며, 검색·필터·정렬도 카테고리 단위로 격리한다. 패키지의 `languages_available` 필드(예: `['ko']` 또는 `['ko','en','ja','zh-TW','zh-HK']`)로 언어별 사이트 카탈로그 노출을 자동 필터링한다. 1Hour·1Pro·꿈길·워크샵은 `['ko']`만 가지므로 외국어 사이트 빌드 시 자동 제외된다.
...
├─ display_currency(enum DisplayCurrency: KRW|USD|JPY|TWD|HKD — C17), display_amount, exchange_rate_at_payment  ※ 결제 시점 스냅샷 — 통화 선호 저장 아님(선호는 쿠키만, §통화 정책)
...
R12 환율 API Free 한도 초과, R15 2FA 디바이스 분실, R16 감사 로그 양 증가(파티셔닝 필요), R17 Cloud Run/CDN 피크 트래픽 청구, R18 Cloud Run Jobs 트랜스코딩 콜드스타트, R19 Apple Relay 이메일 도달, R20 OpenAI/Perplexity API 비용 폭주(하드 캡으로 통제), R21 친구 초대 어뷰즈, R22 외부 리뷰 채널 정책 변동, R23 재가입 7일 쿨다운 우회, R24 중국 본토(간체) 미지원(C14 — 설계상 제외, 대만·홍콩 번체 타겟). 각 항목은 해당 장의 완화책으로 1차 대응하며, 분기 리스크 리뷰에서 등급 상향 여부를 점검한다.
