...
export async function generateMetadata({
  ...
  const t = await getTranslations({
    locale,
    namespace: 'packages.catalog.categories.group',
  });
  return { title: t('metaTitle'), description: t('metaDescription') };
}

export default function GroupPage({ params: { locale } }: { params: { locale: string } }) {
  setRequestLocale(locale);
  return <CategoryCatalog category="group" locale={locale} />;
}
