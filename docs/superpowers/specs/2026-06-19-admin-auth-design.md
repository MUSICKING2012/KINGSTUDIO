...
6. **라우트 레벨 권한 거부 E2E** — 첫 권한-보호 어드민 기능(라우트에 `requirePermission` 적용) 구현 시 **그 슬라이스에서 추가·검증**한다. 현재는 RBAC **단위 테스트**(`hasPermission`·`getAdminPermissions`)까지만 검증됨. `app/admin`에 `requirePermission`이 걸린 보호 라우트가 아직 없어 **라우트 레벨 거부는 미검증(설계상 정상)**. CLAUDE.md §4에도 명시.
7. **세션 분리 E2E 상태 (정직한 기록).** 세션 분리 E2E(고객 쿠키→어드민 라우트 차단, `e2e/admin-auth.spec.ts`)는 **슬라이스 구현 시 통과 확인됨**. 단 이후 **fresh 재실행이 막힌 상태** — Playwright webServer가 이미 떠 있는 dev 서버와 겹쳐 여러 `next dev`가 CPU를 경합한 **환경 문제**(성능 진단상 단일 서버 콜드 컴파일은 ~3–4초로 정상; 컴파일 자체 결함 아님). **테스트 결함 아님** — 환경 정리(dev 서버 1개만, 또는 Playwright webServer를 build+start로 전환) 후 재확인 필요.
