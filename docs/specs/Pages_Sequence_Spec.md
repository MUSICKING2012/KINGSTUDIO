...
| 5e Blog | **결정 ③ ✅ D안(자체 운영·Ghost 배제) + ⓐ~ⓓ 전건 확정(2026-08-05)** / **5e-1 ✅ 구현(PR #40 — /blog·/blog/[slug]·BlogPost 모델·seed:blog·JSON-LD, nav 탭 비활성 유지)**. **5e-2 ✅ 구현(2026-08-07 다산, `feat/blog-5e2-ghost-migration`)** — Ghost 발행 글 99건 이관(press 14·story 74·behind 12, 무태그→story·artistplace→behind Aiden 확정) + 301 맵 99건 + nav 탭 활성 + sitemap 편입. 잔여 = PR 머지 |

## 6. 다음 착수 가이드 (머신 무관 — 새 세션 킥오프용)

> **2026-08-05 갱신:** 5d 는 5d-3(fill-in·전 로케일 전환)까지 구현 완료 — 아래 5d 킥오프 절차는
> **이력 보존용**이다. 현재 잔여 = ① 5e-2(Ghost export 이관·301 맵·nav 탭·sitemap) ② 후속 fill-in 소슬라이스(실사진·
...
보류·백로그: ~~5e Blog 결정 ③ 대기~~ → 확정·5e-1 구현(위 §5). ja·zh 기계번역 감수(service 76 + reviews 17 +
...
- ④ ~~NYT 블로그 글 본문 감수~~ **감수 수행·정정 1건(2026-08-06, 576d743)**: NYT 2024-11
  기사는 강남(선릉역 인근) 시절 취재로 확인 — "성수 스튜디오 방문" 서술을 제거(지역 특정
  삭제, 신규 지역 주장 미발명). 제목 인용(F8)·URL·날짜·클라이언텔 서술은 무결. 홈·어바웃
  NYT 카피도 무결 확인. 오너 최종 확인만 잔여.
- ~~별건: CLAUDE.md §5 로케일 구본(zh-TW 서술) 동기화~~ **완료(2026-08-07 다산)**: §1 스택 표·
