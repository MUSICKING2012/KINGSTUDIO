 ...
 * ③ 렌탈 크로스링크는 /rental 라우트가 살아있는 로케일(ko)에만 링크 — 그 외 비링크
...
for (const locale of LOCALES) {
  test(`/service 렌더 + nav 활성 + 렌탈 게이트 — ${locale}`, async ({ page, request }) => {
    ...
    const rentalOk = (await request.get(`/${locale}/rental`)).status() === 200;
    const rentalLink = page.locator(`main a[href="/${locale}/rental"]`);
